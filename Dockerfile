...
RUN echo "{ \"root_node_ips\": [$(echo $PEER_IPS | awk -F',' '{for(i=1;i<=NF;i++) printf "%s{\"Ip\": \"%s\"}", (i==1?"":","), $i)}], \"try_new_peers\": ${TRY_NEW_PEERS}, \"chain\": \"${CHAIN}\" }" > /home/$USERNAME/override_gossip_config.json
